...
class SearchAndBrowseBackend : public QIviSearchAndBrowseModelInterface
{
    Q_OBJECT
public:
    explicit SearchAndBrowseBackend(QSharedPointer<mopidy::JsonRpcHandler> jrhdl,
                                    QObject *parent = nullptr);

    void initialize() override;
    void fetchData(const QUuid &identifier,
                   const QString &type,
    ...
    QIviPendingReply<void> insert(const QUuid &identifier,
                                  const QString &type,
                                  int index,
                                  const QIviSearchAndBrowseModelItem *item) override;
    QIviPendingReply<void> remove(const QUuid &identifier, const QString &type, int index) override;
    QIviPendingReply<void> move(const QUuid &identifier,
                                const QString &type,
                                int currentIndex,
                                int newIndex) override;
    QIviPendingReply<int> indexOf(const QUuid &identifier,
                                  const QString &type,
                                  const QIviSearchAndBrowseModelItem *item) override;

private Q_SLOTS:
    void onLcSearchResult(const QUuid &identifier, const mopidy::SearchResult &sr);

private:
    QString fieldFromType(const QString &type);
    QHash<QUuid, QString> m_typeOfFetch;
